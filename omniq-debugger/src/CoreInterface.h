...
namespace omniq {
enum class GateType { H, X, Y, Z, CNOT, SWAP, PHASE, RX, RY, RZ, CP };
...
private:
  ...
  MatrixXcd createTwoQubitGate(GateType type, int qubit1, int qubit2,
                               double parameter);

public:
  explicit QuantumState(int numQubits);
  void applyGate(GateType type, int qubit, double parameter = 0.0);
  void applyGate(GateType type, int control, int target,
...
private:
  std::vector<std::tuple<GateType, int, int, double>>
      gates_; // type, qubit1, qubit2, parameter
  int numQubits_;
  int currentStep_;
...
public:
  ...
  QVector<double>
  getQubitState(int qubit) const; // [prob0, prob1, alpha, beta, theta, phi]
  QVector<int> measureQubit(int qubit, int count,
                            const QString &basis = "Z") const;
  ...
  double
  getStateFidelity(const QVector<std::complex<double>> &targetState) const;
...
private:
  ...
  double
  calculateFidelity(const QVector<std::complex<double>> &targetState) const;
