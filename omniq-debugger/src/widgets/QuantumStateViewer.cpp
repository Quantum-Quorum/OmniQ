...
void DensityMatrixWidget::setDensityMatrix(const QVector<QVector<std::complex<double>>> &matrix)
{
...
void DensityMatrixWidget::setStateVector(const QVector<std::complex<double>> &state)
{
    stateVector_ = state;
...
{
    if (densityMatrix_.isEmpty()) return;
...
{
    if (stateVector_.isEmpty()) return;
...
void DensityMatrixWidget::drawBasisLabels(QPainter &painter)
{
    ...
    if (showDensityMatrix_ && !densityMatrix_.isEmpty()) {
        ...
        for (int j = 0; j < size; ++j) {
            int x = startX + j * cellSize;
            painter.drawText(x, startY - 20, cellSize, 20, 
                           Qt::AlignCenter, QString("|%1⟩").arg(j));
        }
        
        // Row labels
        for (int i = 0; i < size; ++i) {
            int y = startY + i * cellSize;
            painter.drawText(startX - 30, y, 30, cellSize, 
...
{
    if (maxValue == 0.0) return Qt::white;
    ...
    , isAnimating_(false)
{
    ...
    connect(animationTimer_, &QTimer::timeout, this, &QuantumStateViewer::onAnimateState);
}
...
void QuantumStateViewer::setupUI()
{
    ...
    viewModeSelector_->addItems({"State Vector", "Density Matrix", "Both"});
    connect(viewModeSelector_, &QComboBox::currentTextChanged,
            this, &QuantumStateViewer::onViewModeChanged);
    ...
    connect(basisSelector_, &QComboBox::currentTextChanged,
            this, &QuantumStateViewer::onBasisChanged);
    controlsLayout->addWidget(basisSelector_);
    
    controlsLayout->addStretch();
    
    animateButton_ = new QPushButton("Animate", this);
    connect(animateButton_, &QPushButton::clicked, this, &QuantumStateViewer::onAnimateState);
    controlsLayout->addWidget(animateButton_);
    
    exportButton_ = new QPushButton("Export", this);
    connect(exportButton_, &QPushButton::clicked, this, &QuantumStateViewer::onExportState);
    ...
    connect(entanglementButton_, &QPushButton::clicked, 
            this, &QuantumStateViewer::onCalculateEntanglement);
    ...
    entanglementTable_->setHorizontalHeaderLabels({"Measure", "Value", "Description"});
    ...
    connect(tomographyButton_, &QPushButton::clicked, 
            this, &QuantumStateViewer::onPerformTomography);
    ...
    tomographyTable_->setHorizontalHeaderLabels({"Measurement", "Expected", "Observed", "Error"});
...
void QuantumStateViewer::updateStateVector(const QVector<std::complex<double>> &stateVector)
{
...
void QuantumStateViewer::updateDensityMatrix(const QVector<QVector<std::complex<double>>> &densityMatrix)
{
...
void QuantumStateViewer::updateEntanglementMeasures(const QMap<QString, double> &measures)
{
    entanglementMeasures_ = measures;
    updateEntanglementTable();
}

void QuantumStateViewer::updateTomographyData(const QVector<double> &tomographyData)
{
...
void QuantumStateViewer::onExportState()
{
    QString fileName = QFileDialog::getSaveFileName(this,
        "Export Quantum State", "", "Text Files (*.txt);;All Files (*)");
    if (!fileName.isEmpty()) {
        QFile file(fileName);
...
void QuantumStateViewer::updateEntanglementTable()
{
    ...
    for (auto it = entanglementMeasures_.begin(); it != entanglementMeasures_.end(); ++it) {
        ...
        entanglementTable_->setItem(row, 1, new QTableWidgetItem(QString::number(it.value(), 'f', 4)));
        
        QString description;
        if (it.key() == "Concurrence") description = "Two-qubit entanglement measure";
        else if (it.key() == "Negativity") description = "Entanglement monotone";
        else if (it.key() == "Von Neumann") description = "Entropy of reduced state";
        else if (it.key() == "Tangle") description = "Three-tangle measure";
...
void QuantumStateViewer::updateTomographyTable()
{
    ...
    for (int i = 0; i < tomographyData_.size() && i < measurements.size(); ++i) {
        ...
        tomographyTable_->setItem(row, 1, new QTableWidgetItem(QString::number(expected[i], 'f', 4)));
        tomographyTable_->setItem(row, 2, new QTableWidgetItem(QString::number(tomographyData_[i], 'f', 4)));
        
        double error = std::abs(tomographyData_[i] - expected[i]);
        tomographyTable_->setItem(row, 3, new QTableWidgetItem(QString::number(error, 'f', 4)));
    }
}
...
void QuantumStateViewer::calculateStateProperties()
{
    ...
    QStringList properties = {"Purity", "Fidelity", "Von Neumann Entropy", "Coherence"};
    ...
    for (int i = 0; i < properties.size(); ++i) {
        ...
        propertiesTable_->setItem(row, 1, new QTableWidgetItem(QString::number(values[i], 'f', 4)));
    }
}
